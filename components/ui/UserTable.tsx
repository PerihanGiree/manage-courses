...
const UserTable: React.FC<PropsType> = ({
	...
	const [deleteModalVisible, setDeleteModalVisible] =
		useState<boolean>(false);
	const [selectedUser, setSelectedUser] = useState<userTypes.User>();
	...
	const handleChangePage = (key: 'increment' | 'decrement'): void => {
		...
		if (key === 'increment') {
			query.offset = offset
				? Number(offset) + Number(lastLimit)
				: lastLimit;
		} else {
			query.offset = offset
				? Number(offset) - Number(lastLimit)
	...
	useEffect(() => {
		...
		if (limit) {
			...
			text =
				(currentPage - 1) * Number(paginationDatas.perPage) +
				1 +
				' - ' +
	...
	const handleEditClick = (item: userTypes.User) => {
				...
				<tbody className="w-full h-[70vh] overflow-auto">
					{users.map((user) => (
						<tr
							...
							key={user.id}
							className="w-full cursor-pointer h-[85px] flex flex-row  bg-white  items-center mt-2 px-3"
						>
							...
							<td className={`${headerTitles[2].className}`}>
								{user.email}
							</td>
							<td className={`${headerTitles[3].className}`}>
								{user.phone}
							</td>
							<td className={`${headerTitles[4].className}`}>
								{user.domain}
							</td>
							...
							<td
								className={`${headerTitles[6].className} text-right pr-4`}
							>
								<button
